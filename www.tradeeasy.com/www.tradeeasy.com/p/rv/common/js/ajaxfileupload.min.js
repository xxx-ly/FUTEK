jQuery.extend({
    createUploadIframe: function(e, t) {
        ...
        return window.ActiveXObject && ("boolean" == typeof t ? o += ' src="javascript:false"' : "string" == typeof t && (o += ' src="' + t + '"')), o += " />", jQuery(o).appendTo(document.body), jQuery("#" + r).get(0)
    },
        ...
        if (r)
            if (jQuery.isArray(r))
                for (var u = 0; u < r.length; u++) {
                    var c = r[u],
                        d = jQuery('<input type="hidden" name="' + c.name + '" />');
                    d.val(c.value), d.appendTo(a)
                } else
                    for (var u in r) {
                        var d = jQuery('<input type="hidden" name="' + u + '" value="' + r[u] + '" />');
                        d.val(r[u]), d.appendTo(a)
                    }
        for (var l = t.split(","), u = 0; u < l.length; u++) {
            var i = jQuery("#" + l[u]),
                y = jQuery(i).clone();
            jQuery(i).attr("id", n + "_" + u), jQuery(i).before(y), jQuery(i).appendTo(a)
        }
        return jQuery(a).css("position", "absolute"), jQuery(a).css("top", "-1200px"), jQuery(a).css("left", "-1200px"), jQuery(a).appendTo("body"), a
    },
    ajaxFileUpload: function(e) {
        ...
        var t = (new Date).getTime(),
            ...
            o = (jQuery.createUploadIframe(t, e.secureuri), "jUploadFrame" + t),
            n = "jUploadForm" + t;
        ...
        var c = function(t) {
            ...
            if (u || "timeout" == t) {
                ...
                e.global && jQuery.event.trigger("ajaxComplete", [u, e]), e.global && !--jQuery.active && jQuery.event.trigger("ajaxStop"), e.complete && e.complete(u, d), jQuery(n).unbind(), setTimeout(function() {
                    try {
                        jQuery(n).remove(), jQuery(r).remove()
                    } catch (t) {
        ...
        try {
            var r = jQuery("#" + n);
            jQuery(r).attr("action", e.url), jQuery(r).attr("method", "POST"), jQuery(r).attr("target", o), r.encoding ? jQuery(r).attr("encoding", "multipart/form-data") : jQuery(r).attr("enctype", "multipart/form-data"), jQuery(r).submit()
        } catch (d) {
    ...
    handleError: function(e, t, r, o) {
        e.error && e.error.call(e.context || window, t, r, o), e.global && (e.context ? jQuery(e.context) : jQuery.event).trigger("ajaxError", [t, e, o])
    }
});